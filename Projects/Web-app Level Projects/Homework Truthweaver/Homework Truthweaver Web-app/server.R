...
function(input, output, session) {
  ...
  observeEvent(
    ...
    handlerExpr = {
      ...
      input_homeworks <- input$homeworks
      file.copy(input_homeworks$datapath, file.path("www", input_homeworks$name), overwrite = TRUE)
    }
  )
  ...
  output$homework_similarity_ranking_table <- renderDataTable(
    {
      homework_similarity_ranking() %>% 
        mutate(Highest_Similarity = Highest_Similarity_Formatted) %>% 
        select(Document, Highest_Similarity, Flag, Other_Document)
    },
  ...
  analytics <- eventReactive(
    ...
    valueExpr = {
      ...
      return(
        box(
          width = 12,
          h2("Analytics") %>% column(width = 12, align = "center"),
          column(
            width = 6,
  ...
  pair_word_clouds <- eventReactive(
    ...
    valueExpr = {
      if (!input$document_1 %in% rownames(df()) | !input$document_2 %in% rownames(df())){
        return(NULL)
      }
      
      document_1_distinct_words <- get_intersect_and_distinct_words(df()[input$document_1, "text"], df()[input$document_2, "text"])$document_1_distinct
      intersect <- get_intersect_and_distinct_words(df()[input$document_1, "text"], df()[input$document_2, "text"])$intersect
      document_2_distinct_words <- get_intersect_and_distinct_words(df()[input$document_1, "text"], df()[input$document_2, "text"])$document_2_distinct
  ...
  output$document_1_distinct_word_clouds <- renderPlot({
    ...
    return(get_wordcloud(document_1_distinct_words, "Greens", 1))
  })
  
  output$documents_intersect_word_clouds <- renderPlot({
    documents_intersect_words <- get_intersect_and_distinct_words(df()[input$document_1, "text"], df()[input$document_2, "text"])$intersect
    
    return(get_wordcloud(documents_intersect_words, "Reds", 1))
  })
  
  output$document_2_distinct_word_clouds <- renderPlot({
    document_2_distinct_words <- get_intersect_and_distinct_words(df()[input$document_1, "text"], df()[input$document_2, "text"])$document_2_distinct
    
    return(get_wordcloud(document_2_distinct_words, "Greens", 1))
  })
  ...
  pair_analysis <- eventReactive(
    ...
    valueExpr = {
      ...
      return(
        box(
          width = 12,
          h2("Pair Analysis") %>% column(width = 12, align = "center"),
          ...
          renderUI({
            if (input$document_1 %in% rownames(df()) & input$document_2 %in% rownames(df()) & input$document_1 != input$document_2) {
              pair_data <- pair_similarity_ranking() %>% 
                filter(Documents == paste(input$document_1, "&", input$document_2) | Documents == paste(input$document_2, "&", input$document_1))
              
              return(
                list(
                  valueBox(
                    width = 12,
                    value = pair_data$Similarity_Formatted %>% paste0("% similar"),
                    subtitle = paste(input$document_1, "&", input$document_2),
                    color = pair_data$Flag %>% tolower(),
                    icon = icon("flag")
                  ),
                  paste0(input$document_1, "'s distinct words:") %>% column(width = 4, align = "center", plotOutput("document_1_distinct_word_clouds")),
                  "Intersect words:" %>% column(width = 4, align = "center", plotOutput("documents_intersect_word_clouds")),
                  paste0(input$document_2, "'s distinct words") %>% column(width = 4, align = "center", plotOutput("document_2_distinct_word_clouds"))
                )
              )
            }
          }),
          renderUI({
            if (input$document_1 %in% rownames(df())){
              document_data <- homework_similarity_ranking() %>% 
                filter(Document == input$document_1)
              
              similar_works <- get_similar_works(pair_similarity_ranking(), input$document_1)
          ...
          renderUI({
            if (input$document_2 %in% rownames(df())){
              document_data <- homework_similarity_ranking() %>% 
                filter(Document == input$document_2)
              
              similar_works <- get_similar_works(pair_similarity_ranking(), input$document_2)
